...
def test_create_todo(client, mocker):
	# spy 특정 객체를 트렉킹하게 된다.
	# spy 를 사용하지 않기 위해서는 equal 매직 메서드를 오버라이딩할 수 있다
	...
	# mocking 을 사용하기 때문에 코드 검증의 누락이 발생할 수 있다
	body = {
		"contents": "FastAPI Section 0",
		"is_done": False
	}
	...
	assert create_spy.spy_return.contents == "FastAPI Section 0"
...
def test_update_todo(client, mocker):
	...
	mocker.patch.object(
			ToDoRepository,
			"update_todo",
			return_value=ToDo(id=1, contents="FastAPI Section 0",
							  is_done=False),
	)

	response = client.patch("/todos/1", json={"is_done": False})

	undone.assert_called_with()
	assert response.status_code == 200
	assert response.json() == {
		"id": 1,
		"contents": "FastAPI Section 0",
